...
export function ClustersPage() {
  ...
  const health = useQuery({
    ...
    queryFn: async () => (await api.get('/api/clusters/health-summary')).data,
  ...
  const create = useMutation({
    ...
    onError: (e) => setErr(e.response?.data?.message ?? 'Register failed'),
  });
  ...
  const remove = useMutation({
    ...
    onError: (e) => alert('Delete failed: ' + (e.response?.data?.message ?? e.message)),
  });

  return (
    <>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        <Typography variant="h4">Manage Clusters</Typography>
      ...
      <Box sx={{
        ...
        {(list.data ?? []).map(c => {
          ...
          const status = h?.status ?? (c.enabled ? 'CRITICAL' : 'DISABLED');
          return (
...
function DeleteClusterButton({
  ...
  return (
    <Stack direction="row" spacing={1} alignItems="center">
      ...
      <IconButton size="small" color="success" disabled={!canFire}
